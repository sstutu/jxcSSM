...
function OnRightClick(event, treeId, treeNode) {//右击方法，在开头的setting里已经做了指定
    if (!treeNode && event.target.tagName.toLowerCase() != "button" && $(event.target).parents("a").length == 0) {
        ...
        gNode=treeNode;//在节点上就获得ztree选中的节点对象
...
function addTreeNode() {//function增加节点的(添加下级和添加跟节点)
    ...
    if(gNode && gNode!=null) {
        $("#myModalLabel").html(gNode.name + "#添加下级");//直接点出它的属性，记住要点出的是menu Bean里面的字段，如果是添加根节点那么，这个gNode上面已经置空为null，点出来也会是null
    }else{
